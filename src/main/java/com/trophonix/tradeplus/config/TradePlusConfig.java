...
public class TradePlusConfig {
    ...
    private final boolean tradeCompatMode = true;
    ...
    private ConfigMessage errorsWaitForExpire, errorsPlayerNotFound, errorsSelfTrade, errorsInvalidUsage, errorsTradingDisabled;
...
//    tradeCompatMode = config.getBoolean("trade-command-compatible-mode", false);
        ...
        myExtraSlots = gui.getStringList("my-extra-slots").stream().map(s -> Integer.valueOf(s)).collect(Collectors.toList());
        theirExtraSlots = gui.getStringList("their-extra-slots").stream().map(s -> Integer.valueOf(s)).collect(Collectors.toList());
        ...
        errorsBlockedWorld = new ConfigMessage(lang, "errors.blocked-world", "&4&l(!) &4You can't trade in this world.");
        ...
        if (accept.getStack() == null) {
            accept = new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "RED_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display("&aClick to Accept").lore(Arrays.asList("&7If either of you change", "&7your offers during the countdown,", "&7you will have to accept", "&7the trade again.")).flag("HIDE_ATTRIBUTES").damage((short) 14).save(gui, "accept.my-icon");
        }
        cancel = new ItemFactory(gui, "accept.my-cancel");
        if (cancel.getStack() == null) {
            cancel = new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "GREEN_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display("&cClick to Cancel").flag("HIDE_ATTRIBUTES").damage((short) 13).save(gui, "accept.my-cancel");
        }

        complete = new ItemFactory(gui, "complete");
        if (complete.getStack() == null) {
            complete = new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "GREEN_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display("&aTrade Complete!").lore(Arrays.asList("&fReview the final agreement and", "&fclose the trade window", "&fto retrieve your items!")).flag("HIDE_ATTRIBUTES").damage((short) 13).save(gui, "complete");
        }

        cancelled = new ItemFactory(gui, "cancelled");
        if (cancelled.getStack() == null) {
            cancelled = new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "RED_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display("&cTrade Cancelled!").lore(Arrays.asList("&7Your trading partner cancelled", "&7the trade. Close the window", "&7to retrieve your items.")).flag("HIDE_ATTRIBUTES").damage((short) 14).save(gui, "cancelled");
        }

        theirAccept = new ItemFactory(gui, "accept.their-icon");
        if (theirAccept.getStack() == null) {
            theirAccept = new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "GREEN_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display("&aThey've accepted your offer.").lore(Arrays.asList("&7If you're satisfied with the", "&7trade as shown right now,", "&7click your accept button!")).flag("HIDE_ATTRIBUTES").damage((short) 13).save(gui, "accept.their-icon");
        }

        theirCancel = new ItemFactory(gui, "accept.their-cancel");
        if (theirCancel.getStack() == null) {
            theirCancel = new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "RED_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display("&aYour partner is still considering.").lore(Arrays.asList("&7Click your accept button to", "&7signal that you like the trade", "&7as it is now, or wait", "&7for them to offer more!")).flag("HIDE_ATTRIBUTES").damage((short) 14).save(gui, "accept.their-cancel");
        }

        separator = new ItemFactory(gui, "separator");
        if (separator.getStack() == null) {
            separator = new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "BLACK_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display(" ").flag("HIDE_ATTRIBUTES").damage((short) 15).save(gui, "separator");
        }

        placeholder = new ItemFactory(gui, "placeholder");
        if (placeholder.getStack() == null) {
            placeholder = new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "BLACK_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display(" ").flag("HIDE_ATTRIBUTES").damage((short) 15).save(gui, "placeholder");
        }
        ...
        if (force.getStack() == null) {
            force = new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "CLOCK" : "WATCH")).display(("&4&lForce Trade")).lore(Arrays.asList("&7Click to force the trade", "&7to countdown and accept as", "&7it stands now.")).flag("HIDE_ATTRIBUTES").save(gui, "force.icon");
        }
    }
    ...
    public void loadConfig() {
        if (configFile.exists()) {
            ...
            config.set("extras.tokenenchant.theirdisplay", "&eTheir TokenEnchants tokens offer is &6%AMOUNT%");
            ...
            config.set("extras.tokenmanager.theirdisplay", "&eTheir TokenManager tokens offer is &6%AMOUNT%");
            ...
            config.set("extras.beasttokens.theirdisplay", "&eTheir TokenManager tokens offer is &6%AMOUNT%");
    ...
    public void loadGui() {
        if (guiFile.exists()) {
            ...
            gui.set("my-slots", Stream.of(1, 2, 3, 9, 10, 11, 12, 18, 19, 20, 21, 27, 28, 29, 30, 36, 37, 38, 39, 45, 46, 47, 48).map(i -> Integer.toString(i)).collect(Collectors.toList()));
            gui.set("their-slots", Stream.of(5, 6, 7, 14, 15, 16, 17, 23, 24, 25, 26, 32, 33, 34, 35, 41, 42, 43, 44, 50, 51, 52, 53).map(i -> Integer.toString(i)).collect(Collectors.toList()));
            ...
            new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "CLOCK" : "WATCH")).display(("&4&lForce Trade")).lore(Arrays.asList("&7Click to force the trade", "&7to countdown and accept as", "&7it stands now.")).flag("HIDE_ATTRIBUTES").save(gui, "force.icon");
            ...
            new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "GREEN_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display("&cClick to Cancel").flag("HIDE_ATTRIBUTES").damage((short) 13).save(gui, "accept.my-cancel");

            new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "GREEN_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display("&aTrade Complete!").lore(Arrays.asList("&fReview the final agreement and", "&fclose the trade window", "&fto retrieve your items!")).flag("HIDE_ATTRIBUTES").damage((short) 13).save(gui, "complete");

            new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "RED_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display("&cTrade Cancelled!").lore(Arrays.asList("&7Your trading partner cancelled", "&7the trade. Close the window", "&7to retrieve your items.")).flag("HIDE_ATTRIBUTES").damage((short) 14).save(gui, "cancelled");

            new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "GREEN_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display("&aThey've accepted your offer.").lore(Arrays.asList("&7If you're satisfied with the", "&7trade as shown right now,", "&7click your accept button!")).flag("HIDE_ATTRIBUTES").damage((short) 13).save(gui, "accept.their-icon");
            ...
            new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "BLACK_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display(" ").flag("HIDE_ATTRIBUTES").damage((short) 15).save(gui, "separator");

            new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "BLACK_STAINED_GLASS_PANE" : "STAINED_GLASS_PANE")).display(" ").flag("HIDE_ATTRIBUTES").damage((short) 15).save(gui, "placeholder");
        }
    }
    ...
    public void update() {
        ...
        if (configVersion < 1.34) {
            ...
            config.set("hooks.economy.youroffer", config.getString("economy.offer", "&7Your money offer is &e%MONEYAMOUNT%"));
        ...
        if (configVersion < 2.0) {
            if (config.contains("messages")) {
                for (String key : config.getConfigurationSection("messages").getKeys(false))
                    lang.set(key, config.getString("messages." + key).replace("%PLAYERNAME%", "%PLAYER%").replace("%MONEYAMOUNT%", "%AMOUNT%").replace("%XPAMOUNT%", "%AMOUNT%"));
        ...
        if (configVersion < 3.70) {
            ...
            config.set("extras.votingplugin.lore", Collections.singletonList("&fClick to edit your offer!"));
            config.set("extras.votingplugin.taxpercent", 0);
        }
        ...
        if (configVersion < 3.75) {
            ...
            ItemFactory placeHolder = new ItemFactory(guiSeparatorType).display(" ").customModelData(guiSeparatorModelData).flag("HIDE_ATTRIBUTES");
            ItemFactory acceptTrade = new ItemFactory(guiCancelType, Material.EMERALD).display(guiAcceptDisplay).customModelData(guiAcceptModelData).flag("HIDE_ATTRIBUTES");
            ItemFactory cancelTrade = new ItemFactory(guiAcceptType, Material.REDSTONE).display(guiCancelDisplay).customModelData(guiCancelModelData).flag("HIDE_ATTRIBUTES");
            ItemFactory theyAccepted = new ItemFactory(guiAcceptType, Material.EMERALD).display(guiTheirAcceptDisplay).customModelData(guiAcceptModelData).flag("HIDE_ATTRIBUTES");
            ItemFactory theyCancelled = new ItemFactory(guiCancelType, Material.REDSTONE).display(guiTheirCancelDisplay).customModelData(guiCancelModelData).flag("HIDE_ATTRIBUTES");
            ItemFactory force = new ItemFactory(guiForceType, Sounds.version < 113 ? Material.getMaterial("WATCH") : Material.getMaterial("CLOCK")).display(guiForceName).lore(guiForceLore).flag("HIDE_ATTRIBUTES");
